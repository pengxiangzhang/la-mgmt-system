class Appointment::StudentRequestController < ApplicationController
  ...
  def create
    if Course.find_by_course_name(params['class_id']).nil? || !params['method'].in?(['In Person', 'Online']) || !params['duration'].to_i.between?(5, 60)
      flash[:error] = 'You are not allow to do this.'
